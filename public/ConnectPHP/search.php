...
class Constants
{
    ...
    //STATEMENTS
    static $SQL_SELECT_ALL="SELECT * FROM `search`, `report` WHERE `search`.`reportid` = `report`.`id` and `search`.`username` = '.$name.'";
        ...
        if($con != null)
        {
            ...
            if($result->num_rows>0)
            {
                ...
                while($row=$result->fetch_array())
                {
                    ...
                    "image_url"=>$row['image'],"create"=>$row['createreport'],"solve"=>$row['solve'],"completed"=>$row['completed'],));
                }
